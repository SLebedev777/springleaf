...
good_features_list=joblib.load('good_features_list.pkl')
...
for cfr_name in Classifiers.keys():
    ...
    for t in range(T):
        ...
        mean_acc/= Q
        #if cfr_name=='SVM': print('C=',cfr.C, 'mean acc=%0.2f'%mean_acc,'mean AUC=%0.2f'%mean_auc)
        cv_results.append((mean_fpr,mean_tpr,mean_auc,mean_acc,probas))
        #plt.plot(mean_fpr, mean_tpr, '--',label=cfr_name+'Mean ROC (area = %0.2f)' % mean_auc, lw=2)   
